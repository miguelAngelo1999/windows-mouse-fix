...
echo.
echo === Building WmfVirtualPad UMDF2 driver (x64) ===
echo.
...
set WDF_LIB=%WDK_ROOT%\Lib\wdf\umdf\x64\%UMDF_VER%
...
lib /nologo /DEF:"%OUTDIR%\ucrt.def" /OUT:"%OUTDIR%\ucrt.lib" /MACHINE:X64
if %ERRORLEVEL% NEQ 0 (echo FAILED to create ucrt.lib & exit /b 1)

echo Compiling WmfVirtualPad.c...
cl.exe /nologo /c /MD /D_AMD64_ /DAMD64 /DWIN32_LEAN_AND_MEAN=1 /D_WIN32_WINNT=0x0A00 /DWINVER=0x0A00 /DWINNT=1 /DNTDDI_VERSION=0x0A00000C /D_WINDLL /D_UNICODE /DUNICODE /DUMDF_VERSION_MAJOR=2 /DUMDF_VERSION_MINOR=33 /DUMDF_USING_NTSTATUS /I"%WDF_INC%" /I"%WDK_INC%\shared" /I"%WDK_INC%\um" /I"%WDK_INC%\ucrt" /I"%WDK_INC%\km" /I"..\shared" /FI"%WDK_INC%\shared\warning.h" /W3 /WX- /O2 /GS- /Gy /Fo"%OUTDIR%\WmfVirtualPad.obj" WmfVirtualPad.c
if %ERRORLEVEL% NEQ 0 (echo COMPILE FAILED & exit /b 1)

echo Linking...
link.exe /nologo /DLL /OUT:"%OUTDIR%\WmfVirtualPad.dll" /MACHINE:X64 /SUBSYSTEM:WINDOWS /NODEFAULTLIB "%OUTDIR%\WmfVirtualPad.obj" "%WDF_LIB%\WdfDriverStubUm.lib" "%WDK_LIB%\um\x64\mincore.lib" "%WDK_LIB%\um\x64\ntdllp.lib" "%MSVC_LIB%\msvcrt.lib" "%MSVC_LIB%\vcruntime.lib" "%OUTDIR%\ucrt.lib"
